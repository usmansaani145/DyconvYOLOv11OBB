#!/usr/bin/env python3
"""Evaluate ORCNN on ship_obb_v2 val set."""
...
CONFIG = Path(__file__).resolve().parent / "config_orcnn_v2.py"
...
def main() -> int:
    ...
    work_dir = RUNS_DIR / "orcnn" / f"orcnn_seed{seed}"
    ...
    if pkl_path.is_file() and not force_infer:
        ...
        subprocess.run(cmd, cwd=str(work_dir), check=True, env=mmrotate_env(f"./tmp/mpl_orcnn"))
    ...
    run_unified_eval(
        ...
        f"ORCNN seed{seed} (ship_obb_v2)",
        str(work_dir / "metrics_orcnn.json"),
    )
    return 0
