#!/usr/bin/env python3
"""Combo 3 — DyConv + EMA + MSDA (full). Seed via argv[1]."""
...
from models.build_combo3 import apply_combo3_injections, build_combo3  # noqa: E402

sys.path.insert(0, str(REPO_ROOT))
from shared.obb_coco_eval import run_coco_eval  # noqa: E402

COMBO_NAME = "combo3_dyconv_msda_ema"
...
def run_training(seed: int) -> Dict[str, Any]:
  ...
  print(f"Combo3 DyConv+EMA+MSDA | seed={seed} | GPU={torch.cuda.get_device_name(0)}")

  model = build_combo3(verbose=True)
  if not check_activation_scale(model, "Combo3"):
    raise RuntimeError("Activation scale check failed — fix before training")

  def on_train_start(trainer):
    print("\n=== on_train_start: Combo3 injection + gamma optimizer ===")
    apply_combo3_injections(trainer.model, verbose=True)
