#!/usr/bin/env python3
"""Idea 2 — EMA at YOLOv11-OBB backbone tail. Seed via argv[1]."""
...
IDEA_ROOT = REPO_ROOT / "ideas" / "idea2_ema"
sys.path.insert(0, str(IDEA_ROOT))


from models.inject_ema import build_idea2_model, inject_ema_module  # noqa: E402
...
def run_training(seed: int) -> Dict[str, Any]:
  ...
  print(f"Idea2 EMA | seed={seed}")
  model = build_idea2_model(WEIGHTS, verbose=True)

  gate_cb = _make_gate_log_callback()
  model.add_callback("on_train_batch_end", make_nan_stop_callback())
  model.add_callback(
      "on_train_start",
      make_injection_start_callback(inject_ema_module),
  )
