#!/usr/bin/env python3
"""Combo 1 — DyConv + MSDA. Seed via argv[1]."""
...
from models.build_combo1 import apply_combo1_injections, build_combo1  # noqa: E402

sys.path.insert(0, str(REPO_ROOT))
from shared.obb_coco_eval import run_coco_eval  # noqa: E402

COMBO_NAME = "combo1_dyconv_msda"
...
def run_training(seed: int) -> Dict[str, Any]:
  ...
  print(f"Combo1 DyConv+MSDA | seed={seed} | GPU={torch.cuda.get_device_name(0)}")

  model = build_combo1(verbose=True)
  if not check_activation_scale(model, "Combo1"):
    raise RuntimeError("Activation scale check failed — fix before training")

  def on_train_start(trainer):
    print("\n=== on_train_start: Combo1 injection + gamma optimizer ===")
    apply_combo1_injections(trainer.model, verbose=True)
